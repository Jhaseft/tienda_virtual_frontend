...
import { fetchStoreById, fetchStoreProducts } from "@/app/(explorarTienda)/api/public-explorarTienda.api"
    ...
    <main className="min-h-screen bg-gray-50 pt-4 md:pt-20">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-8 pb-28 md:pb-12">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <Suspense>
            <ProductsSearchInput storeId={id} initialValue={q} />
          </Suspense>
        </div>
        ...
        {products.length > 0 ? (
          ...
          <div className="flex flex-col items-center py-20 gap-2 text-gray-400">
            ...
            {q && (
              <Link href={`/tiendas/${id}/productos`} className="text-xs text-violet-600 hover:underline">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2">
            {currentPage > 1 ? (
              <Link
                href={`/tiendas/${id}/productos?page=${currentPage - 1}`}
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                <Link
                  key={p}
                  href={`/tiendas/${id}/productos?page=${p}`}
