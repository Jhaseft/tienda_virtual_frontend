...
export default function AdminSidebar() {
  ...
  return (
    <aside className="hidden md:flex w-64 shrink-0 flex-col bg-white border-r border-gray-100 h-screen sticky top-0 overflow-y-auto shadow-sm">
        ...
        {ITEMS.map(({ href, label, Icon, activeBg, activeText, color }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all border ${
                active
            ...
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 transition-all ${
                active ? "bg-white shadow-sm" : "bg-gray-50 group-hover:bg-white group-hover:shadow-sm"
              }`}>
